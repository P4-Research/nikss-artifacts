...
REGISTER_TABLE(bridged_headers, BPF_MAP_TYPE_PERCPU_ARRAY, u32, struct headers, 1)
BPF_ANNOTATE_KV_PAIR(bridged_headers, u32, struct headers)
...
__builtin_memcpy(data + 14, &xdp2tc_md, sizeof(struct xdp2tc_metadata));
        ...
        if (parsed_hdr->ethernet.ebpf_valid) {
            ...
            u64 dstAddr_val = parsed_hdr->ethernet.dstAddr;
            dstAddr_val = htonll(dstAddr_val << 16);
            ebpf_byte = ((char*)(&dstAddr_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&dstAddr_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&dstAddr_val))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char*)(&dstAddr_val))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char*)(&dstAddr_val))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char*)(&dstAddr_val))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            u64 srcAddr_val = parsed_hdr->ethernet.srcAddr;
            srcAddr_val = htonll(srcAddr_val << 16);
            ebpf_byte = ((char*)(&srcAddr_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&srcAddr_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&srcAddr_val))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char*)(&srcAddr_val))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_byte = ((char*)(&srcAddr_val))[4];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 4, (ebpf_byte));
            ebpf_byte = ((char*)(&srcAddr_val))[5];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 5, (ebpf_byte));
            ebpf_packetOffsetInBits += 48;

            u16 etherType_val = parsed_hdr->ethernet.etherType;
            etherType_val = bpf_htons(etherType_val);
            ebpf_byte = ((char*)(&etherType_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&etherType_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;
