...
struct ingress_as_key_cache {
    u32 group_ref;
    u32 field0; /* headers.ipv4.src_addr */
    u16 field1; /* local_metadata.l4_sport */
} __attribute__((aligned(4)));
...
struct ingress_tbl_routing_value_cache {
    struct ingress_tbl_routing_value value;
    u8 hit;
};
...
REGISTER_TABLE(ingress_as_cache, BPF_MAP_TYPE_LRU_HASH, struct ingress_as_key_cache, struct ingress_as_value, 512)
BPF_ANNOTATE_KV_PAIR(ingress_as_cache, struct ingress_as_key_cache, struct ingress_as_value)
...
REGISTER_TABLE(ingress_tbl_routing_cache, BPF_MAP_TYPE_LRU_HASH, struct ingress_tbl_routing_key, struct ingress_tbl_routing_value_cache, 512)
BPF_ANNOTATE_KV_PAIR(ingress_tbl_routing_cache, struct ingress_tbl_routing_key, struct ingress_tbl_routing_value_cache)
...
;
            ...
            if (hit_3) {
                unsigned int action_run = 0;
                {
                    ...
                    /* perform lookup */
                    struct ingress_tbl_routing_value_cache* cached_value = NULL;
                    cached_value = BPF_MAP_LOOKUP_ELEM(ingress_tbl_routing_cache, &key);
                    if (cached_value != NULL) {
                        ...
                        if (value != NULL) {
                            struct ingress_tbl_routing_value_cache cache_update = {0};
                            cache_update.hit = hit_3;
                            __builtin_memcpy((void *) &(cache_update.value), (void *) value, sizeof(struct ingress_tbl_routing_value));
                            BPF_MAP_UPDATE_ELEM(ingress_tbl_routing_cache, &key, &cache_update, BPF_ANY);
                        }
                    }
                    if (value != NULL) {
                        /* run action */
                        ...
                        if (value->ingress_as_is_group_ref != 0) {
                            key_cache.group_ref = value->ingress_as_ref;
                            key_cache.field0 = headers->ipv4.src_addr;
                            key_cache.field1 = local_metadata->l4_sport;
                        ...
                        if (as_value != NULL && do_update_cache != 0) {
                            BPF_MAP_UPDATE_ELEM(ingress_as_cache, &key_cache, as_value, BPF_ANY);
                        }
                        if (as_value != NULL) {
...
__builtin_memcpy(data + 14, &xdp2tc_md, sizeof(struct xdp2tc_metadata));
        ...
        if (headers->ipv4.ebpf_valid) {
            ...
            u8 ver_ihl_val = headers->ipv4.ver_ihl;
            ebpf_byte = ((char*)(&ver_ihl_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            u8 diffserv_val = headers->ipv4.diffserv;
            ebpf_byte = ((char*)(&diffserv_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            u16 total_len_val = headers->ipv4.total_len;
            ...
            u16 identification_val = headers->ipv4.identification;
            ...
            u16 flags_offset_val = headers->ipv4.flags_offset;
            flags_offset_val = bpf_htons(flags_offset_val);
            ebpf_byte = ((char*)(&flags_offset_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&flags_offset_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u8 ttl_val = headers->ipv4.ttl;
            ebpf_byte = ((char*)(&ttl_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            u8 protocol_val = headers->ipv4.protocol;
            ebpf_byte = ((char*)(&protocol_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_packetOffsetInBits += 8;

            u16 hdr_checksum_val = headers->ipv4.hdr_checksum;
            ...
            u32 src_addr_val = headers->ipv4.src_addr;
            ...
            u32 dst_addr_val = headers->ipv4.dst_addr;
        ...
        if (headers->tcp.ebpf_valid) {
            ...
            u16 sport_val = headers->tcp.sport;
            sport_val = bpf_htons(sport_val);
            ebpf_byte = ((char*)(&sport_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&sport_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u16 dport_val = headers->tcp.dport;
            dport_val = bpf_htons(dport_val);
            ebpf_byte = ((char*)(&dport_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&dport_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u32 seq_no_val = headers->tcp.seq_no;
            seq_no_val = htonl(seq_no_val);
            ebpf_byte = ((char*)(&seq_no_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&seq_no_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&seq_no_val))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char*)(&seq_no_val))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

            u32 ack_no_val = headers->tcp.ack_no;
            ack_no_val = htonl(ack_no_val);
            ebpf_byte = ((char*)(&ack_no_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&ack_no_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_byte = ((char*)(&ack_no_val))[2];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 2, (ebpf_byte));
            ebpf_byte = ((char*)(&ack_no_val))[3];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 3, (ebpf_byte));
            ebpf_packetOffsetInBits += 32;

            u8 data_offset_val = headers->tcp.data_offset;
            ebpf_byte = ((char*)(&data_offset_val))[0];
            write_partial(pkt + BYTES(ebpf_packetOffsetInBits) + 0, 4, 4, (ebpf_byte >> 0));
            ebpf_packetOffsetInBits += 4;

            u8 res_val = headers->tcp.res;
            ebpf_byte = ((char*)(&res_val))[0];
            write_partial(pkt + BYTES(ebpf_packetOffsetInBits) + 0, 3, 1, (ebpf_byte >> 0));
            ebpf_packetOffsetInBits += 3;

            u8 ecn_val = headers->tcp.ecn;
            ...
            u8 ctrl_val = headers->tcp.ctrl;
            ebpf_byte = ((char*)(&ctrl_val))[0];
            write_partial(pkt + BYTES(ebpf_packetOffsetInBits) + 0, 6, 0, (ebpf_byte >> 0));
            ebpf_packetOffsetInBits += 6;

            u16 window_val = headers->tcp.window;
            window_val = bpf_htons(window_val);
            ebpf_byte = ((char*)(&window_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&window_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u16 checksum_val = headers->tcp.checksum;
            checksum_val = bpf_htons(checksum_val);
            ebpf_byte = ((char*)(&checksum_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&checksum_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u16 urgent_ptr_val = headers->tcp.urgent_ptr;
            urgent_ptr_val = bpf_htons(urgent_ptr_val);
            ebpf_byte = ((char*)(&urgent_ptr_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&urgent_ptr_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;
        ...
        if (headers->udp.ebpf_valid) {
            ...
            u16 sport_val = headers->udp.sport;
            sport_val = bpf_htons(sport_val);
            ebpf_byte = ((char*)(&sport_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&sport_val))[1];
            ...
            u16 dport_val = headers->udp.dport;
            dport_val = bpf_htons(dport_val);
            ebpf_byte = ((char*)(&dport_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&dport_val))[1];
            ...
            u16 len_val = headers->udp.len;
            len_val = bpf_htons(len_val);
            ebpf_byte = ((char*)(&len_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&len_val))[1];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 1, (ebpf_byte));
            ebpf_packetOffsetInBits += 16;

            u16 checksum_val = headers->udp.checksum;
            checksum_val = bpf_htons(checksum_val);
            ebpf_byte = ((char*)(&checksum_val))[0];
            write_byte(pkt, BYTES(ebpf_packetOffsetInBits) + 0, (ebpf_byte));
            ebpf_byte = ((char*)(&checksum_val))[1];
        ...
        headers->__helper_variable = ostd.egress_port;
        BPF_MAP_UPDATE_ELEM(bridged_headers, &ebpf_zero, &(*headers), BPF_ANY);
...
int xdp_egress_func(struct xdp_md *skb) {
    ...
    pkt_len += 4;
    struct headers_t *headers;
    headers = BPF_MAP_LOOKUP_ELEM(bridged_headers, &ebpf_zero);
    if (!headers) {
        return XDP_DROP;
    }
    __u32 egress_ifindex = headers->__helper_variable;
    struct hdr_md *hdrMd;
    ...
    u8 headers_bridged_meta_ingress_ebpf_valid = headers->bridged_meta.ebpf_valid;
    u8 headers_ethernet_ingress_ebpf_valid = headers->ethernet.ebpf_valid;
    u8 headers_vlan_tag_ingress_ebpf_valid = headers->vlan_tag.ebpf_valid;
    u8 headers_ipv4_ingress_ebpf_valid = headers->ipv4.ebpf_valid;
    u8 headers_tcp_ingress_ebpf_valid = headers->tcp.ebpf_valid;
    u8 headers_udp_ingress_ebpf_valid = headers->udp.ebpf_valid;
    headers->bridged_meta.ebpf_valid = 0;
    headers->ethernet.ebpf_valid = 0;
    headers->vlan_tag.ebpf_valid = 0;
    headers->ipv4.ebpf_valid = 0;
    headers->tcp.ebpf_valid = 0;
    headers->udp.ebpf_valid = 0;
    start: {
/* extract(headers->bridged_meta) */
        if (!headers_bridged_meta_ingress_ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 32 + 0)) {
                ebpf_errorCode = PacketTooShort;
                goto reject;
            }
...
/* extract(headers->ethernet) */
        if (!headers_ethernet_ingress_ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 112 + 0)) {
                ebpf_errorCode = PacketTooShort;
                goto reject;
            }
    ...
    parse_vlan: {
/* extract(headers->vlan_tag) */
        if (!headers_vlan_tag_ingress_ebpf_valid) {
            if (ebpf_packetEnd < pkt + BYTES(ebpf_packetOffsetInBits + 32 + 0)) {
                ebpf_errorCode = PacketTooShort;
                goto reject;
            }
